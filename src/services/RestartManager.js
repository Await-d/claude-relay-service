...
class RestartManager extends EventEmitter {
  ...
  createLogger() {
    ...
    return {
      error: (msg, ...args) => {
        if (currentLevel >= 0) console.error(`[RestartManager ERROR]`, msg, ...args)
      },
      warn: (msg, ...args) => {
        if (currentLevel >= 1) console.warn(`[RestartManager WARN]`, msg, ...args)
      },
      info: (msg, ...args) => {
        if (currentLevel >= 2) console.log(`[RestartManager INFO]`, msg, ...args)
      },
      debug: (msg, ...args) => {
        if (currentLevel >= 3) console.log(`[RestartManager DEBUG]`, msg, ...args)
      }
    }
  }
  ...
  async postStartupValidation(pid) {
  ...
  updateMetrics(success, restartTime) {
    ...
    if (success) {
      ...
      // 更新平均重启时间
      const totalTime = this.metrics.averageRestartTime * (this.metrics.successfulRestarts - 1) + restartTime
