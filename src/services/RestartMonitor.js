...
class RestartMonitor extends EventEmitter {
  ...
  createLogger() {
    ...
    return {
      error: (msg, ...args) => {
        if (currentLevel >= 0) {
          console.error(`[RestartMonitor ERROR]`, msg, ...args)
        }
      },
      warn: (msg, ...args) => {
        if (currentLevel >= 1) {
          console.warn(`[RestartMonitor WARN]`, msg, ...args)
        }
      },
      info: (msg, ...args) => {
        if (currentLevel >= 2) {
          console.log(`[RestartMonitor INFO]`, msg, ...args)
        }
      },
      debug: (msg, ...args) => {
        if (currentLevel >= 3) {
          console.log(`[RestartMonitor DEBUG]`, msg, ...args)
        }
      }
    }
  }
  ...
  parseMetricLogEntry(line) {
    const match = line.match(
      /\[(.+?)\] strategy=(\w+) duration=(\d+)ms success=(true|false) pid=(.+)/
    )
    if (!match) {
      return null
    }
  ...
  getPerformanceStats() {
    ...
    for (const [strategy, records] of Object.entries(strategies)) {
      ...
      stats.byStrategy[strategy] = {
        ...
        averageDuration:
          strategyDurations.length > 0
            ? Math.round(strategyDurations.reduce((a, b) => a + b, 0) / strategyDurations.length)
            : 0,
  ...
  getAlertHistory(limit = 50) {
    return this.metrics.alertHistory.slice(-limit).reverse() // 最新的在前
  }
