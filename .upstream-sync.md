...
**同步日期**: 2025-12-16
**上游版本**: v1.1.235
**上游提交**: `79f2cebd` - chore: sync VERSION file with release v1.1.235 [skip ci]
**本地标记**: 待创建（建议 `sync-upstream-v1.1.235-20251216`）
**同步提交**: 待提交（当前工作区已完成合并，尚未推送）

### 同步内容

- ✅ API Key 与用户统计总费用改用 allTimeCost，避免月度键过期导致总费用偏低（#801）
- ✅ Console 转发使用 header 白名单 `filterForClaude`，规避 CDN 头触发上游安全检查
- ✅ 前端调整：新增 Opus 4.5 快捷映射按钮，受限模型列表支持滚动（溢出修复）
- 🔄 VERSION 保持本地 1.1.238（未回退上游 1.1.235）
...
- `npm run lint:check`
- `npm test`
- `npm run build:web`（存在 console.warn 输出，构建成功）
- `NODE_ENV=test npm start`（25s 超时前启动成功并优雅退出；data 目录不可写，价格缓存未落盘）
